...
def print_placement(a_placement):
	"""print a placement nicely"""
	puzzle_width = int(len(a_placement)**0.5)
	
	for row in range(0,puzzle_width):
		print_header(puzzle_width)
		print("|",end='')
		for column in range(0,puzzle_width):
			card=return_card(a_placement,row*3+column)
...
def try_card(a_placement, a_card, an_untried_cards, a_puzzle_width):		
	...
	if matches(a_placement, a_card, a_puzzle_width): #default orientation
		...
		solve1(a_placement[:], an_untried_cards[:], a_puzzle_width)
		
def solve1(a_placement, an_untried_cards, a_puzzle_width):
	#print("Placement Length: " +str(len(a_placement)))
	#print_placement(a_placement)
	if len(a_placement) == a_puzzle_width**2: #if we have a completed placement, we're done
		...
		for card in an_untried_cards:
			...
			try_card(a_placement[:], card, remaining_cards, a_puzzle_width)
			try_card(a_placement[:], rotate_card(card), remaining_cards, a_puzzle_width)
			try_card(a_placement[:], rotate_card(rotate_card(card)), remaining_cards, a_puzzle_width)
			try_card(a_placement[:], rotate_card(rotate_card(rotate_card(card))), remaining_cards, a_puzzle_width)
...
def solve(a_cardset):
	...
	solve1(placement, a_cardset, puzzle_width)
